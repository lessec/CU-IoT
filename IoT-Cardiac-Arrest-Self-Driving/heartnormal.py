...
def stdin_listener():
    while True:
        selection = input("\n---Enter e(exit) or q(quit) to quit---\n")
        if selection == "e" or selection == "E" or selection == "exit" or selection == "Exit" or selection == "EXIT" or selection == "q" or selection == "Q" or selection == "quit" or selection == "Quit" or selection == "QUIT":
            print("\n--> Quitting Heart Rate Simulator...")
            break
...
async def main():
    ...
    # Organise location information value with Null data (Protect privacy)
    geojson = {"lat": None, "lon": None, "alt": None}
    ...
    # Organise driver information value with Null data (Protect privacy)
    properties_driver_info = helper.create_reported_properties(
        ...
        driverName="Jane Doe",
        bloodyType=None,
        currentStatus="Lovely Condition",
        currentSituation="Good",
    )

    # Send serial number, heart rate, device information, location and driver information to Device of IoT Central
    ...
    # Generate heart rate values as random: normal values (60-100)
    async def send_telemetry():
        print("\n-->Start to sending heart rate simulating value to IoT Central")
        while True:
            curr_hr_ext = random.randrange(60, 101)
