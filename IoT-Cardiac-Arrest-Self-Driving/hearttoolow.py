...
def stdin_listener():
    while True:
        selection = input("\n---Enter any key to continue simulation---\n")
        if selection != None:
            break
...
async def main():
    ...
    # Generate heart rate values as random: too low values (1-39: loss of consciousness)
    async def send_telemetry():
        print("\n-->Start to sending heart rate simulating value to IoT Central")
        while True:
            curr_hr_ext = random.randrange(1, 40)
            ...
            print("--> If want continue, enter any key")
            send_telemetry.cancel()  # Sending it only one time, cause' driver lost consciousness
